...
var staticData = {
    ...
    amount: "Total appreciation during run",
    com_nam: "run",
    pty: "can_par_aff",
    url: "data/scatter_expenditure.csv"
};
//alert(staticData.date);
...
function loadData() {
    ...
    d3.csv(sources.static.url, function (csv) {
        ...
        sources.static.data.forEach(function (d) {
            ...
            // Use regex to clean strings as needed for our keys
            var can = vizuly.util.createCSSKey(String(d[sources.static.name]));
...
function onUpdate() {
    ...
    //viz_title.transition().text("Total: " + formatCurrency(Math.round(total)) +"%"); + Math.round(total) +"%"
	viz_title.transition().text("Dividend runs");
}

// On mouse over we select all plots related to the candidate ID and highlight them.
function onMouseOver(e, d, i) {
    viz.selection().selectAll(".vz-scatter-node." + d.key).style("fill", "#0f2747").style("fill-opacity", .8).style("opacity", 1);
