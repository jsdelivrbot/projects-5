...
(function() {
    ...
    d3.parsets = function() {
        ...
        function parsets(selection) {
            selection.each(function(data, i) {
                ...
                function updateDimensions() {
                    ...
                    // dimension
                    //     .call(d2.behavior.click()
                    //       .origin(identity)
                    //       .on("click"), function(d){
                    //         d3.select("#listView2").text(d).append('br');
                    //       })
                    dimension
                        .call(d3.behavior.drag()
                            ...
                            .on("drag", function(d) {
                                ...
                                for (var i = 1; i < dimensions.length; i++) {
                                    ...
                                    //d3.select("#listView2").text(i).append('br');
                                }
                                d3.select(this)
        ...
        function clicked(d, i) {
            var sl = 1;
            var numOfRow = d.count;
            var nameIs = d.name;
            var dimensionName = d.dimension.name;
            //d3.select("#listView2").text("Clicked" + " " + numOfRow + " " + i + " " + nameIs + " " + dimensionName);
            d3.select("table").remove();
            var tabulate = function(data, columns) {
                var table = d3.select('#listView2').append('table')
                				.attr("cellspacing",0)
                				.attr("cellpadding",5)
                				.attr("border",1);
                ...
                rows.selectAll('tr')
                    ...
                    .text(function(d) {
                        // var rex = /([0-9\.]+)\b/g;
                        // if(d.value == nameIs || d.value.match(rex))
                        return d.value;
                    });
                // .style("text-align","center")
                // .style("padding-left","10px")
            ...
            d3.csv('titanic.csv', function(data) {
                //var newData = data.slice(0, numOfRow);
                if (dimensionName == "Willingness") {
                    data2 = data.filter(function(d) {
                        return d.Willingness == nameIs
                    });
                    var columns = ['names'];
                    tabulate(data2, columns);
                } else if (dimensionName == "Sustainability") {
                    data2 = data.filter(function(d) {
                        return d.Sustainability == nameIs
                    });
                    var columns = ['names'];
                    tabulate(data2, columns);
                } else if (dimensionName == "Outlook") {
                    data2 = data.filter(function(d) {
                        return d.Outlook == nameIs
                    });
                    var columns = ['names'];
                    tabulate(data2, columns);
                }
    ...
    function buildTree(root, data, dimensions, value) {
        ...
        for (var i = 0; i < n; i++) {
            ...
            //d3.select("#listView2").appendtext(node.count +"\n");
    ...
    function defaultTooltip(d) {
        ...
        while (d.parent) {
            if (d.name) {
                path.unshift(d.name);
                // d3.select("#listView2").text(count +"\n");
            }

            d = d.parent;
        }
        //d3.select("#listView").text(path.join(" → ") + " " + comma(count) + " (" + percent(count / d.count) + ")");
        //listName = "";
        // for(var i = d.count- count + 1; i <= d.count; i++)
        //   listName += " "+i;
        // d3.select("#listView2").text( listName);
        //listName = '';
    ...
    function defaultCategoryTooltip(d) {
        var listName = "";
        //d3.select("#listView").text(d.name + " " + comma(d.count) + " (" + percent(d.count / d.dimension.count) + ")");
        //listName = "";
        // for(var i = d.dimension.count - d.count + 1; i <= d.dimension.count; i++)
        //   listName += " "+i;
        // d3.select("#listView2").text( listName);
