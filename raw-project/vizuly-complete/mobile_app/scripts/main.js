...
var chartColors=["#9cbb59","#B4CCD9","#7986CB","#2DA4A9","#E57373"];
var labelColors=["#B4CCD9","#539997","#2D4FAC","#3F5874","#C62828"];

// Data Fields
var dataFields = ["Volume","Open","Close","High","Low"];
var menuItems = [{label: "Open", value: "Open"},{label: "Close", value: "Close"},{label: "High", value: "High"},{label: "Low", value: "Low"},{label: "Volume", value: "Volume"}];
...
function initialize() {
    ...
    d3.selectAll(".introItem")
        .style("width",listWidth + "px")
        .style("top",function(d,i) { return (chartHeight * i) + "px"});
    ...
    chartItems[0].forEach(function (item,i) {
        ...
        var chart=vizuly.viz.linearea_mobile(item)
            ...
            .dataLabel(function (d) {
                return (dataFields[i] == "Volume") ? volumeFormat(this.y(d)) : currFormat(this.y(d));
            })
            .on("validate",function () {
                this.component.yScale(d3.scale.pow());
            })
            .on("measure",function () {
                if (dataFields[i] == "Volume") {
